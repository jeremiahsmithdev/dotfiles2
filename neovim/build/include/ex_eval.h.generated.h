...
DLLEXPORT int aborted_in_try(void);
DLLEXPORT _Bool cause_errthrow(const char_u *mesg, _Bool severe, _Bool *ignore) FUNC_ATTR_NONNULL_ALL;
DLLEXPORT void free_global_msglist(void);
DLLEXPORT void do_errthrow(cstack_T *cstack, char_u *cmdname);
DLLEXPORT int do_intthrow(cstack_T *cstack);
DLLEXPORT char *get_exception_string(void *value, except_type_T type, char_u *cmdname, int *should_free);
...
DLLEXPORT int has_loop_cmd(char_u *p);
#include "nvim/func_attr.h"
