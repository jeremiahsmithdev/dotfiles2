...
#  endif
#endif
DLLEXPORT uint64_t extmark_set(buf_T *buf, uint64_t ns_id, uint64_t *idp, int row, colnr_T col, int end_row, colnr_T end_col, Decoration *decor, _Bool right_gravity, _Bool end_right_gravity, ExtmarkOp op);
DLLEXPORT _Bool extmark_del(buf_T *buf, uint64_t ns_id, uint64_t id);
DLLEXPORT _Bool extmark_clear(buf_T *buf, uint64_t ns_id, int l_row, colnr_T l_col, int u_row, colnr_T u_col);
DLLEXPORT ExtmarkInfoArray extmark_get(buf_T *buf, uint64_t ns_id, int l_row, colnr_T l_col, int u_row, colnr_T u_col, int64_t amount, _Bool reverse);
DLLEXPORT ExtmarkInfo extmark_from_id(buf_T *buf, uint64_t ns_id, uint64_t id);
...
DLLEXPORT void extmark_adjust(buf_T *buf, linenr_T line1, linenr_T line2, long amount, long amount_after, ExtmarkOp undo);
