...
DLLEXPORT void ns_hl_def(NS ns_id, int hl_id, HlAttrs attrs, int link_id);
DLLEXPORT int ns_get_hl(NS ns_id, int hl_id, _Bool link, _Bool nodefault);
DLLEXPORT _Bool win_check_ns_hl(win_T *wp);
DLLEXPORT int hl_get_ui_attr(int idx, int final_id, _Bool optional);
...
DLLEXPORT Dictionary hl_get_attr_by_id(Integer attr_id, Boolean rgb, Error *err);
DLLEXPORT Dictionary hlattrs2dict(HlAttrs ae, _Bool use_rgb);
DLLEXPORT HlAttrs dict2hlattrs(Dictionary dict, _Bool use_rgb, int *link_id, Error *err);
