...
DLLEXPORT void set_expr_line(char_u *new_line);
DLLEXPORT char_u *get_expr_line(void);
DLLEXPORT char_u *get_expr_line_src(void);
...
DLLEXPORT _Bool get_spec_reg(int regname, char_u **argp, _Bool *allocated, _Bool errmsg);
DLLEXPORT _Bool cmdline_paste_reg(int regname, _Bool literally_arg, _Bool remcr);
DLLEXPORT int op_delete(oparg_T *oap);
DLLEXPORT int op_replace(oparg_T *oap, int c);
...
DLLEXPORT void init_yank(void);
DLLEXPORT void free_register(yankreg_T *reg) FUNC_ATTR_NONNULL_ALL;
DLLEXPORT _Bool op_yank(oparg_T *oap, _Bool message, int deleting) FUNC_ATTR_NONNULL_ALL;
...
DLLEXPORT char_u *skip_comment(char_u *line, _Bool process, _Bool include_space, _Bool *is_comment);
DLLEXPORT int do_join(size_t count, int insert_space, int save_undo, int use_formatoptions, _Bool setmark);
DLLEXPORT void op_format(oparg_T *oap, int keep_cursor);
DLLEXPORT void op_formatexpr(oparg_T *oap);
DLLEXPORT int fex_format(linenr_T lnum, long count, int c);
DLLEXPORT void format_lines(linenr_T line_count, int avoid_fex);
DLLEXPORT int paragraph_start(linenr_T lnum);
...
DLLEXPORT void write_reg_contents(int name, const char_u *str, ssize_t len, int must_append);
DLLEXPORT void write_reg_contents_lst(int name, char_u **strings, _Bool must_append, MotionType yank_type, colnr_T block_len);
DLLEXPORT void write_reg_contents_ex(int name, const char_u *str, ssize_t len, _Bool must_append, MotionType yank_type, colnr_T block_len);
...
DLLEXPORT _Bool op_reg_set_previous(const char name) FUNC_ATTR_WARN_UNUSED_RESULT;
