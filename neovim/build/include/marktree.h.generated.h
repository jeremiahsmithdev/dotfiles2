...
#  endif
#endif
DLLEXPORT uint64_t marktree_put(MarkTree *b, int row, int col, _Bool right_gravity, uint8_t decor_level);
DLLEXPORT uint64_t marktree_put_pair(MarkTree *b, int start_row, int start_col, _Bool start_right, int end_row, int end_col, _Bool end_right, uint8_t decor_level);
DLLEXPORT void marktree_put_key(MarkTree *b, int row, int col, uint64_t id);
DLLEXPORT void marktree_del_itr(MarkTree *b, MarkTreeIter *itr, _Bool rev);
DLLEXPORT void marktree_clear(MarkTree *b);
DLLEXPORT void marktree_free_node(mtnode_t *x);
DLLEXPORT uint64_t marktree_revise(MarkTree *b, MarkTreeIter *itr, uint8_t decor_level);
DLLEXPORT void marktree_move(MarkTree *b, MarkTreeIter *itr, int row, int col);
DLLEXPORT _Bool marktree_itr_get(MarkTree *b, int row, int col, MarkTreeIter *itr);
...
DLLEXPORT mtmark_t marktree_itr_current(MarkTreeIter *itr);
DLLEXPORT _Bool marktree_splice(MarkTree *b, int start_line, int start_col, int old_extent_line, int old_extent_col, int new_extent_line, int new_extent_col);
...
DLLEXPORT mtpos_t marktree_lookup(MarkTree *b, uint64_t id, MarkTreeIter *itr);
